...
flights_df['Airline'] = flights_df['Airline'].apply(lambda airline: combine_duplicates(airline))

# Copying the dataframes before adding columns and editing the original df

# Save copy for clustering df
flights_clustering_df = flights_df.copy()

# Duplicate df for clustering and classification df
flights_class_df = flights_df.copy()
...
flights_df['Routes Amount'] = flights_df['Route'].apply(lambda route: count_routes(route))
...
flights_df['Routes as stops'] = flights_df['Total_Stops'].apply(lambda stops: count_stops(stops))
print("Is Route and Total_Stops columns are indicate the same data? ",
      flights_df['Routes Amount'].equals(flights_df['Routes as stops']))
